      ...
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-2 text-sm">
          <span
            ...
            :class="dashboard.wsConnected ? 'bg-emerald-400 animate-pulse' : 'bg-slate-500'"
          ></span>
          <span :class="dashboard.wsConnected ? 'text-emerald-400' : 'text-slate-400'">
            {{ dashboard.wsConnected ? 'WebSocket 已連線' : '未連線' }}
          </span>
        </div>
    ...
    <main class="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4 p-4">
      ...
      <div class="bg-slate-800 rounded-2xl p-5 flex flex-col gap-4">
        ...
        <div class="flex-1 flex flex-col items-center justify-center">
          <div
            class="text-7xl font-black transition-all duration-200"
            :class="dashboard.currentNote ? dashboard.noteColor : 'text-slate-700'"
          >
            {{ dashboard.currentNote ?? '—' }}
          </div>
          <div class="text-slate-500 text-sm mt-2">{{ dashboard.currentInstrument }}</div>
        </div>

        <!-- Enable audio -->
        <button
          v-if="!dashboard.audioEnabled"
      ...
      <div class="bg-slate-800 rounded-2xl p-5 flex flex-col gap-3">
        <div class="flex items-center justify-between">
          <h2 class="text-slate-400 text-xs uppercase tracking-widest font-semibold">音符串流</h2>
        ...
        <div class="flex-1 overflow-y-auto space-y-1.5 pr-1">
          <transition-group name="feed">
            <div
              v-for="event in dashboard.recentNotes"
              :key="event.id"
              class="flex items-center justify-between bg-slate-700/60 rounded-lg px-3 py-2 text-sm"
            >
              <span class="font-bold" :class="dashboard.noteColor">{{ event.note }}</span>
              <span class="text-slate-400 text-xs">{{ event.instrument }}</span>
              <span class="text-slate-500 text-xs">{{ event.time }}</span>
            </div>
          </transition-group>
          <p v-if="!dashboard.recentNotes.length" class="text-slate-600 text-sm text-center py-8">
      ...
      <div class="bg-slate-800 rounded-2xl p-5 flex flex-col gap-3">
        <div class="flex items-center justify-between">
          ...
          <button
            @click="dashboard.fetchRecentGestures()"
        ...
        <div class="flex-1 overflow-y-auto space-y-1.5 pr-1">
          <div
            v-for="g in dashboard.recentGestures"
            :key="g.id"
          ...
          <p v-if="!dashboard.recentGestures.length" class="text-slate-600 text-sm text-center py-8">
            尚無紀錄
          </p>
        </div>
